...
# from fedml_experiments.standalone.fedavg.main import FedAvgAPI
    ...
    def optimzie_attack_object(self,clients):
        ...
        if self.args.attacker_knowlege in self.atk_knowlege:
            ...
            if self.args.attacker_knowlege in self.min_sum_atk:
                ...
                self.pre_obj = cur_obj
                if cur_obj == self.max_sum_distance :return True
                else:return False
    ...
    def dnc_attack(self,clients):
        if self.args.attack_type == "model_attack":
            ...
            for c in new_clients:
                poisoned.append(RAG.vectorize_state_dict(c.grad, self.device))
            poisoned_mean = torch.mean(torch.stack(poisoned))
            delta_mp = torch.norm(poisoned_mean-self.vec_fedavg,2).item()
            wandb.log({"delta_mp":delta_mp,"round_idx":self.round_idx})
            return new_clients
    def a_little_attack(self,clients):
        if self.args.attack_type == "model_attack":
            if self.args.attacker_knowlege == "a_little":
                ...
                vec_grad = []
                for c in clients:
                    vec_grad.append(c.vec_grad_one*c.grad_norm)
                ...
                for i,c in enumerate(clients):
                    if c.byzantine:
                        ...
                        c.vec_grad_one = poisoned_vec_grad/poisoned_grad_norm
                        c.vec_grad_no_meanvar = RAG.vectorize_weight(poisoned_grad,self.device)
                        c.grad = poisoned_grad
                        c.grad_norm = poisoned_grad_norm
                        # if i==1:
        ...
        for client in clients:
            if client.byzantine:
                for key, va in avg_dict.items():
                    ...
                    # print("vv")
                    # round_idx, self.local_training_data, self.device, self.args, w_global, self.pre_local_model
                client.grad = self.model_trainer.pess_train(self.round_idx,self.args, self.device,atk,client,clients)
