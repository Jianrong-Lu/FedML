...
def get_dataloader(dataset, data_dir, train_bs, test_bs, device,client_idx=None):
...
def load_partition_data_federated_emnist(dataset, data_dir, batch_size,device):
    ...
    for client_idx in range(DEFAULT_TRAIN_CLIENTS_NUM):
        train_data_local, test_data_local = get_dataloader(dataset, data_dir, batch_size, batch_size, device,client_idx)
