...
def get_dataloader(dataset, datadir, train_bs, test_bs, device,dataidxs=None):
    return get_dataloader_CIFAR100(datadir, train_bs, test_bs, device,dataidxs)
...
def get_dataloader_CIFAR100(datadir, train_bs, test_bs, device,dataidxs=None):
    ...
    # train_dl = MultiEpochsDataLoader(train_ds, batch_size=train_bs,
    #                                shuffle=True, num_workers=2,
    #                                pin_memory=False)
    # test_dl = MultiEpochsDataLoader(test_ds, batch_size=test_bs,
...
def load_centralized_data_cifar100(dataset, data_dir, partition_method, partition_alpha, client_number, args,device):
    X_train, y_train, X_test, y_test, net_dataidx_map, traindata_cls_counts = partition_data(dataset,
                                                                                             data_dir,
                                                                                             partition_method,
                                                                                             client_number,
    ...
    train_data_global, test_data_global = get_dataloader(dataset, data_dir, args.train_bs, args.test_bs,device)
    logging.info("train_dl_global number = " + str(len(train_data_global)))
    logging.info("test_dl_global number = " + str(len(train_data_global)))
    ...
    for client_idx in range(client_number):
        ...
        # training batch size = 64; algorithms batch size = 32
        train_data_local, test_data_local = get_dataloader(dataset, data_dir,args.train_bs, args.test_bs,device,
                                                 dataidxs)
...
def load_partition_data_cifar100(dataset, data_dir, partition_method, partition_alpha, client_number, batch_size,device):
    X_train, y_train, X_test, y_test, net_dataidx_map, traindata_cls_counts = partition_data(dataset,
                                                                                             data_dir,
                                                                                             partition_method,
                                                                                             client_number,
    ...
    train_data_global, test_data_global = get_dataloader(dataset, data_dir, batch_size, batch_size,device)
    logging.info("train_dl_global number = " + str(len(train_data_global)))
    logging.info("test_dl_global number = " + str(len(train_data_global)))
    test_data_num = len(test_data_global)
    ...
    for client_idx in range(client_number):
        ...
        # training batch size = 64; algorithms batch size = 32
        train_data_local, test_data_local = get_dataloader(dataset, data_dir,batch_size, batch_size,device,
                                                 dataidxs)
        logging.info("client_idx = %d, batch_num_train_local = %d, batch_num_test_local = %d" % (
            client_idx, len(train_data_local), len(test_data_local)))
        train_data_local_dict[client_idx] = train_data_local
