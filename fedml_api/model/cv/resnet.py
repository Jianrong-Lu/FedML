...
__all__ = ['ResNet','resnet20','resnet110']
...
class Bottleneck_gn(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        # Both self.conv2 and self.downsample layers downsample the input when stride != 1
        self.conv1 = conv1x1(inplanes, width)
        self.bn1 = norm_layer(num_groups=2,num_channels=width)
        self.conv2 = conv3x3(width, width, stride, groups, dilation)
        self.bn2 = norm_layer(num_groups=2,num_channels=width)
        self.conv3 = conv1x1(width, planes * self.expansion)
        self.bn3 = norm_layer(num_groups=2,num_channels=planes * self.expansion)
        self.relu = nn.ReLU(inplace=True)
        self.downsample = downsample
        self.stride = stride
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                ...
                norm_layer(num_groups=2, num_channels= planes * block.expansion)
...
def resnet20_gn(class_num, pretrained=False, path=None, **kwargs):
    model = ResNet(Bottleneck_gn, [3, 3, 3], class_num,  **kwargs)
    if pretrained:
...
def resnet20(class_num, pretrained=False, path=None, **kwargs):
    ...
    if pretrained:
        ...
        model.load_state_dict(new_state_dict)
    return model
def moon_resnet20(class_num, pretrained=False, path=None, **kwargs):
    """
    Constructs a ResNet-110 model.
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained.
    """
    model = ResNet(Bottleneck, [3, 3, 3], class_num,KD = True, **kwargs)
    if pretrained:
        checkpoint = torch.load(path)
        state_dict = checkpoint['state_dict']
