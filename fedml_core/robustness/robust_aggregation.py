...
def grad_dict_aggregate(args,device,clients):
    ...
    return global_grad
def model_dict_aggregate(args,device,clients):
    global_model = collections.OrderedDict()
    for key,_ in clients[0].grad.items():
        for i, c in enumerate(clients):
            if i == 0:
                local_grad_params = c.cur_local_model[key].to(device).mul(c.weight)
            else:
                local_grad_params += c.cur_local_model[key].to(device).mul(c.weight)
        global_model[key] = local_grad_params
    return global_model

def delta_c_dict_aggregate(args,device,clients):
    global_c = collections.OrderedDict()
    for key,_ in clients[0].local_c.items():
        for i, c in enumerate(clients):
            if i == 0:
                local_grad_params = c.local_c[key].to(device).mul(c.weight)
            else:
                local_grad_params += c.local_c[key].to(device).mul(c.weight)
        global_c[key] = local_grad_params
    return global_c
...
class RobustAggregator(object):
    ...
    def fedavg(self,round_idx, clients, current_model, args, device):
        ...
        return fedavg_dict
    def feddyn(self,round_idx, clients, current_model, args, device):
        for c in clients:
            c.weight = 1/args.client_num_per_round #设置聚合权重
        model_dict_avg = model_dict_aggregate(args, device, clients)
        for c in clients:
            c.weight = 1#设置聚合权重
        model_dict_sum = model_dict_aggregate(args, device, clients)
        return model_dict_avg,model_dict_sum
    def fedss(self,round_idx, clients, current_model, args, device):
        summ = 0
        for c in clients:
            summ += c.local_sample_number
        for c in clients:
            c.weight = (c.local_sample_number/summ)**2
        fedavg_dict = grad_dict_aggregate(args, device, clients)
        return fedavg_dict
    def mime(self,round_idx, clients, current_model, args, device):
        for c in clients:
            c.weight = 1/args.client_num_per_round
        fedavg_update_dict = grad_dict_aggregate(args, device, clients)
        # fedavg_model_dict = model_dict_aggregate(args, device, clients)
        fedavg_model_dict =None
        norm_client_full_grad = []
        for i,c in enumerate(clients):
            c.cur_local_model = c.full_grad
            norm_client_full_grad.append(torch.square(torch.norm(vectorize_state_dict(copy.deepcopy(c.full_grad),device),2)))
        fedavg_full_grad_for_m = model_dict_aggregate(args, device, clients)
        # central_full_grad = model_dict_aggregate(args, device, clients)

        global_norm = torch.square(torch.norm(vectorize_state_dict(copy.deepcopy(fedavg_full_grad_for_m),device),2))
        # gamma_t0 = torch.mean(torch.stack(norm_client_full_grad).mul(1/global_norm))
        # print("norm_client_full_grad",norm_client_full_grad)
        # print("global_norm",global_norm)
        # wandb.log({"mean_of_gamma_t0":gamma_t0.item(), "round": round_idx})
        return fedavg_full_grad_for_m,fedavg_update_dict,fedavg_model_dict,global_norm

    def scaffold(self, round_idx, clients, current_model, args, device):

        for c in clients:
            # print(c.client_idx,c.labels)
            c.weight = 1 / args.client_num_per_round
            # c.weight = c.weight/weight_sum
        # print(grad_dict_aggregate(args,device,clients))
        fedavg_dict = grad_dict_aggregate(args, device, clients)
        delta_c_dict = delta_c_dict_aggregate(args,device,clients)
        ...
        return fedavg_dict,delta_c_dict
    def fedparc(self, round_idx, clients, current_model, args, device):
        sample_sum = 0
        batch_loss_mean = 0
        batch_grad_mean = collections.OrderedDict()

        for c in clients:
            c.weight =  1 /args.client_num_per_round
        #     batch_loss_mean += c.batch_loss * c.batch_num/sample_sum
        # print("sample_sum", sample_sum)
        fedavg_dict = grad_dict_aggregate(args, device, clients)
        return fedavg_dict,batch_grad_mean,batch_loss_mean
    def fedTye(self, round_idx, clients, current_model, args, device):

        sample_sum = 0
        batch_loss_mean = 0
        batch_grad_mean = collections.OrderedDict()
        for key, _ in clients[0].batch_grad.items():
            for i, c in enumerate(clients):
                if i == 0:
                    local_grad_params = c.batch_grad[key].to(device).mul(1/args.client_num_per_round)
                else:
                    local_grad_params += c.batch_grad[key].to(device).mul(1/args.client_num_per_round)
            batch_grad_mean[key] = local_grad_params
        for c in clients:
            c.weight =  1 /args.client_num_per_round
            batch_loss_mean += c.batch_loss * c.batch_num/sample_sum
        print("sample_sum", sample_sum)
    ...
    def fedBT(self,round_idx, clients, current_model, args, device):

        sample_sum = 0
        central_loss = 0
        for c in clients:
            sample_sum += c.batch_num
            # central_loss += c.delta_loss *c.batch_num

        for c in clients:
            c.weight =  c.batch_num/sample_sum
            # c.weight = args.init_r * 1 /args.client_num_per_round
        # central_loss = central_loss/sample_sum
        print("sample_sum",sample_sum)
        fedavg_dict = grad_dict_aggregate(args, device, clients)
        # fedavg_vect = vectorize_state_dict(copy.deepcopy(fedavg_dict),device)

        return fedavg_dict
    def fedGG(self,round_idx,clients,atk_state,cur_model,args,device,model_trainer):
        # module B
        if args.dataset == "mnist":
            ...
            for i,c in enumerate(clients):
                ...
                if i in [9,19,12,2,6]:
                    print(c.client_idx, c.labels)
                    group_client[0].append(c)
                if i in [16,10,18,13,14]:
                    print(c.client_idx, c.labels)
                    group_client[1].append(c)
                if i in [15,7,5,3,17]:
                    print(c.client_idx, c.labels)
                    group_client[2].append(c)
        ...
        for i,c in enumerate(clients):
            i = i+1
            # if i in [9,19,12,2,6,16,10,18,13,14,15,7,5,3,17]:
            # iid_group_vec_local_danwh_grads.append(c.vec_grad_one)
            iid_group_vec_local_danwh_grads.append(c.vec_grad_no_meanvar)
            norm.append(c.grad_norm)
        vec_avg_grad = torch.mean(torch.stack(iid_group_vec_local_danwh_grads), dim=0)
        vec_avg_norm = torch.mean(torch.stack(norm), dim=0)
        gradients.append(vec_avg_grad)
        norms.append(vec_avg_norm)
        # global_gradient = torch.mean(torch.stack(gradients), dim=0).mul(torch.mean(torch.stack(norms), dim=0)).cpu().tolist()
        global_gradient = torch.mean(torch.stack(gradients), dim=0).cpu().tolist()
        #########

        #########
        # gradients = []
        # norms = []
        # for key,clients in group_client.items():
        #     iid_group_vec_local_danwh_grads = []
        #     norm = []
        #     for c in clients:
        #         iid_group_vec_local_danwh_grads.append(c.vec_grad_one)
        ...
        for key, value in clients[0].grad.items():
            temp = global_gradient[:value.numel()]
            global_grad_dict[key] = torch.tensor(temp).view(value.size()).to(device)
            del global_gradient[:value.numel()]
        return global_grad_dict
    def fedal(self,round_idx, clients, current_model, args, device):
        norm_grad_vec = []
        for c in clients:  # 把所有被选中的用户的动量单位化
            norm_grad_vec.append(c.momentum)
        ...
        # wandb.log({"dis_f_m": dis.item(), "round": round_idx})
        return global_grad_dict
    def fedavgM(self,round_idx, clients, current_model, args, device):
        for c in clients:
            c.weight = 1/args.client_num_per_round
            # c.weight = c.weight/weight_sum
        # print(grad_dict_aggregate(args,device,clients))
    ...
    def MAB_FL(self, round_idx,clients,atk_state,cur_model,args,device,model_trainer):
        ...
        print("The graphs:",C)
        print("The largest graph{}:".format(C[0],C[0]))  # 最大的
        if (len(C[0]) > 1):
            for i in C[0]:
